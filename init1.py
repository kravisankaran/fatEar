...
def search():
    if request.method == "POST":
        ...
        cursor = conn.cursor()
        cursor.execute(
            "select s.title, a.fname, a.lname, s.releaseDate from song s join artistPerformsSong asp on s.songID = asp.songID join artist a on a.artistID = asp.artistID where title like %s or a.fname like %s and a.lname like %s ",
            (song, artistFName, artistLName))
        conn.commit()
        data = cursor.fetchall()
        print(data)
        ...
        if song is None or song == "":
            ...
            if artistFName != "":
                if artistLName != "":
                    ...
                    cursor.execute(
                        "select s.title, a.fname, a.lname, s.releaseDate from song s join artistPerformsSong asp on s.songID = asp.songID join artist a on a.artistID = asp.artistID where a.fname like %s and a.lname like %s ",
                ...
                if artistLName == "":
                    ...
                    cursor.execute(
                        "select s.title, a.fname, a.lname, s.releaseDate from song s join artistPerformsSong asp on s.songID = asp.songID join artist a on a.artistID = asp.artistID where a.fname like %s ",
                        (artistFName))
                    conn.commit()
                    data = cursor.fetchall()
            if artistFName == "":
                if artistLName != "":
                    ...
                    cursor.execute(
                        "select s.title, a.fname, a.lname, s.releaseDate from song s join artistPerformsSong asp on s.songID = asp.songID join artist a on a.artistID = asp.artistID where a.lname like %s ",
                        (artistLName))
                    conn.commit()
                    data = cursor.fetchall()
        if song != "":
            ...
            if song == 'all':
                ...
                cursor.execute(
                    "select s.title, a.fname, a.lname, s.releaseDate from song s join artistPerformsSong asp on s.songID = asp.songID join artist a on a.artistID = asp.artistID")
                conn.commit()
                data = cursor.fetchall()
                print(data)
